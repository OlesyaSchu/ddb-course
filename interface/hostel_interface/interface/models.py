...
class PlaceType(Model):  # Таблица 3. Тип места
    ...
    PRICE = SmallIntegerField()  # SmallInt
...
class Guests(Model):  # Таблица 9. Гости
    guest_id = BigAutoField(primary_key=True, unique=True)  # UUID  PRIMARY KEY,
...
class Bookings(Model):  # Таблица 5. Брони
    ...
    arrival_date = DateField()  # TIMESTAMP WITH TIME ZONE NOT NULL,
    checkout_date = DateField()  # TIMESTAMP WITH TIME ZONE,
...
class LoginUsers(Model):  # Таблица. Пользователи
    service_id = BigAutoField(primary_key=True)  # UUID  PRIMARY KEY,
    login = CharField(max_length=15)  # LOGIN,
    password = CharField(max_length=100)  # PASSWORD VARCHAR(15)
#
#
